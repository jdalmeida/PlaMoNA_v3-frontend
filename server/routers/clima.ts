...
export const climaRouter = createTRPCRouter({
  ...
  getClima: publicProcedure.query(async () => {
    try {
      ...
      const clima = await axios.get(`${baseUrl}/current.json?key=${apiKey}&q=Venâncio&lang=pt`);
  ...
  getPrevisao: publicProcedure.query(async () => {
    try {
      const apiKey = process.env.WEATHER_API_KEY;
      const baseUrl = process.env.WEATHER_BASE_URL || 'http://api.weatherapi.com/v1';
